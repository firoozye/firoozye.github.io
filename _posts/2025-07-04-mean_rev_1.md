...
## Nyblom-M\"ak\"al\"ainen Test and KPSS test
...
where typically we assume \\(\epsilon_t\sim \mathcal{N}(1,\sigma_\epsilon^2)\\) and 
...
The test statistic is computed as follows: we estimate the state-space model, and then compute the residualsu
\\(\hat{\epsilon}_t\\) from the model.
...
![Trading Framework](images/Flow_cw.png)
